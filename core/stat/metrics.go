...
func NewMetrics(name string) *Metrics {
	...
	return &Metrics{
		executor:  executors.NewPeriodicalExecutor(time.Minute, container),
		container: container,
	}
}
...
func (c *metricsContainer) Execute(v any) {
	...
	report := &StatReport{
		...
		Pid:           c.pid,
		ReqsPerSecond: float32(size) / float32(time.Minute/time.Second),
		Drops:         drops,
	}
