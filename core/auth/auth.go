...
// /////////////////对称加密//////////////////////
func Issue(data map[string]interface{}, secretKey []byte) (string, error) {
...
func Auth(signedToken string, secretKey []byte) (map[string]interface{}, error) {
...
// /////////////////非对称加密//////////////////////
func IssueRSA(data map[string]interface{}, privateKeyPem string) (string, error) {
	claims := MyCustomClaims{
		data,
		jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(2 * time.Hour)),
...
func AuthRSA(signedToken string, publicKeyPem string) (map[string]interface{}, error) {
	token, err := jwt.ParseWithClaims(signedToken, &MyCustomClaims{}, func(t *jwt.Token) (interface{}, error) {
